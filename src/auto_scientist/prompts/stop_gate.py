...
STOP_PERSONAS: list[dict[str, str]] = [
    {
        ...
        "system_text": (
            ...
            "- 'The goal asks about nonlinear effects, but only one functional\n"
            "   form was tested. Standard nonlinearity analyses for this domain\n"
            "   include saturating, piecewise, and interaction effects.'\n"
            "- 'The goal asks about confounding variables, but the investigation\n"
            "   never tested for measurement artifacts or sensor ceiling effects.'\n"
            "- 'Temperature effects on the process were mentioned in the goal\n"
            "   but never quantified in any iteration.'\n"
            "\n"
    ...
    },
    {
        ...
        "system_text": (
            ...
            "\n"
            "Example concerns in your lane:\n"
            '- \'The investigation concluded "no nonlinear effects" after\n'
            "   testing only a quadratic polynomial. Saturating (Michaelis-\n"
            "   Menten), piecewise, and interaction effects are common in\n"
            "   this domain and were not tested.'\n"
            "- 'Missingness was declared random based on correlation with\n"
            "   one variable. Ceiling effects, sensor saturation, and\n"
            "   conditional missingness patterns were not checked.'\n"
            "- 'A causal edge was dismissed via linear Granger test, but\n"
            "   the relationship could be threshold-based or nonlinear,\n"
            "   which Granger cannot detect.'\n"
            "\n"
            "NOT in your lane (belongs to other persona):\n"
        ...
        "instructions": (
            ...
            "   - Was only one functional form tested when multiple are common?\n"
            "   - Was only one statistical method used when alternatives exist?\n"
            ...
            "   functional forms, and common failure modes for the methods used.\n"
            "\n"
...
# ---------------------------------------------------------------------------
# Scientist Stop Defense (in debate)
# ---------------------------------------------------------------------------

STOP_SCIENTIST_DEBATE_SYSTEM = """\
<role>
You are a scientist defending your decision to stop an investigation during
a critique debate. You have web search available to support your claims
with evidence.
</role>

<pipeline_context>
You proposed stopping this investigation. A completeness assessment
identified gaps, and a critic is challenging your decision based on those
gaps. You must defend your claim that the investigation is complete, or
acknowledge that further work is needed.

Dismissing a gap as "peripheral" requires justification: explain why
further investigation of that gap would not meaningfully improve the
answer to the investigation goal.
</pipeline_context>

<instructions>
1. Address each concern from the critic. For each gap:
   - If the gap is genuinely peripheral, explain why remaining iterations
     would not meaningfully improve the answer.
   - If the gap is valid, acknowledge it.

2. Do not dismiss gaps with vague language like "future work" unless you
   can explain why the current answer is sufficient without investigating
   that gap.

3. Be honest about the limits of what was investigated. If a single
   negative result was used to close a sub-question, defend why that
   single test is sufficient or acknowledge that it is not.

4. Use web search to support your defense with evidence.
</instructions>
...
<data>
<your_stop_reason>{stop_reason}</your_stop_reason>
<critique persona="{critic_persona}">{critique}</critique>
</data>

<task>
Defend your decision to stop the investigation against the critic's
challenges. Address each concern. If a gap is valid, acknowledge it.
</task>
"""
