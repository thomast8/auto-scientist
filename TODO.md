...
- [x] Checkpoint replay: `auto-scientist replay --from <run> --at-iteration N` copies a saved run, rewinds state/notebook/artifacts to iteration N, and re-runs from there. Iteration manifests persist per-panel TUI metadata (tokens, timing, summaries) so future replays can reconstruct collapsed panels for preserved iterations. - 2026-03-28
