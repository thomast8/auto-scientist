...
from auto_core.scheduler import wait_for_window
from auto_core.sdk_backend import CODEX_MODEL_OVERRIDES
from auto_core.state import ExperimentState, VersionEntry
from auto_core.summarizer import run_with_summaries, summarize_results
from auto_core.validation import validate_prerequisites
from auto_core.widgets import AGENT_STYLES, AgentPanel, console

logger = logging.getLogger(__name__)

# Default from RunConfig, used when self.config is None (e.g. tests)
_DEFAULT_RUN_TIMEOUT = RunConfig.model_fields["run_timeout_minutes"].default
...
class Orchestrator:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            self._live.stop()
            try:
                from auto_core.sdk_backend import close_all_backends

                await close_all_backends()
            except Exception:
                logger.debug("Backend cleanup failed", exc_info=True)
            try:
                from auto_core.sdk_backend import cleanup_sessions
    ...
    async def _run_iteration_body(self) -> None:
        ...
        from auto_core.resume import AGENT_ORDER
