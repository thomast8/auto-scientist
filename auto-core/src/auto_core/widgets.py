...
# Agent style palette - populated by `auto_core.roles.install(RoleRegistry(...))`
# at app bootstrap. Each app (auto-scientist, auto-reviewer) owns its own
# palette; this dict is the core's lookup table.
AGENT_STYLES: dict[str, str] = {}

# Short descriptions shown immediately when an agent panel opens (before the
# first summary). Populated by `auto_core.roles.install(...)`.
