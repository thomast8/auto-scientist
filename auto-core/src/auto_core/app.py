...
class PipelineCommandProvider(Provider):
    ...
    async def search(self, query: str) -> Hits:
        ...
        for panel in app.query(AgentPanel):
            commands.append(
                (
                    f"View {panel.display_panel_name} details",
...
class PipelineApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="outer-container") as outer:
            outer.border_title = widgets.APP_LABEL
    ...
    def _on_panel_collapsed(self, panel: AgentPanel) -> None:
        ...
        self.notify(f"{panel.display_panel_name} complete ({elapsed})")
...
class ShowApp(App):
    ...
    def on_mount(self) -> None:
        ...
        if saved_theme in self.available_themes:
            self.theme = saved_theme
        self.title = f"{widgets.APP_LABEL} - {self._run_title}"
    ...
    def action_open_focused_detail(self) -> None:
        ...
        while widget is not None:
            if isinstance(widget, AgentPanel):
                self.push_screen(
                    AgentDetailScreen(
                        panel_name=widget.display_panel_name,
                        model=widget.model,
