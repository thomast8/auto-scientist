...
from auto_core.sdk_backend import SDKBackend
...
class OutputValidationError(RetryValidationError):
    ...
    Subclasses :class:`~auto_core.retry.ValidationError` so that the
...
def _get_deferred_tool_descriptions() -> dict[str, str]:
    ...
    from auto_core.agents._mcp_base import get_deferred_descriptions
